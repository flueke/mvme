...
static void TEST_listfilter_extractor(benchmark::State &)
{
    // single extractor, 16 bit, not reversed, wordcount=2, repetitions=4
    {
        ...
        assert(ce.output.data.size == (1u << (8 + 2)));
        ...
        assert(dcmp(ce.output.data[0x001], 0x1111));
        assert(dcmp(ce.output.data[0x102], 0x2222));
        assert(dcmp(ce.output.data[0x203], 0x3333));
        assert(dcmp(ce.output.data[0x308], 0x8888));
    }
    ...
    // single extractor, 16 bit, not reversed, wordcount=2, repetitions=4, repetition forms low address bits
    {
        ...
        assert(dcmp(ce.output.data[(0x01u << 2) | 0x0 ], 0x1111));
        assert(dcmp(ce.output.data[(0x02u << 2) | 0x1 ], 0x2222));
        assert(dcmp(ce.output.data[(0x03u << 2) | 0x2 ], 0x3333));
        assert(dcmp(ce.output.data[(0x08u << 2) | 0x3 ], 0x8888));
    }
...
static void BM_listfilter_extractor(benchmark::State &state)
{
    ...
    assert(ce.output.data.size == (1u << (4 + 2)));
