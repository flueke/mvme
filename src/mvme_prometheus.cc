...
{
    auto promInstance = get_prometheus_instance();
    spdlog::info("StreamProcCountersPromExporter: prometheus context instance is {}", fmt::ptr(promInstance.get()));
    auto promRegistry = promInstance->registry();
    d = std::make_unique<Private>(*promRegistry);
}
