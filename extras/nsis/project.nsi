...
; Note (f.lueke@mesytec.com):
; This file was originally generated by CPACK as part of a MVP build (the
; MDPP16 flash tool). I modified it to produce an installer for mvme.
; The files to be installed are expected to be in a "deploy" directory on the
; build machine (see DEPLOY_DIR below).
...
;Include Modern UI
  ...
  ;InstallDir "$PROGRAMFILES64\mvme"
...
;General
  ...
  Name "mvme"
  !ifdef OutDir
    OutFile "${OutDir}/mvme-installer-${ARCH}-${VERSION}.exe"
  !else
    OutFile "mvme-installer-${ARCH}-${VERSION}.exe"
  !endif
...
!macro InitSection SecName
  ...
  ReadRegDWORD $AR_RegFlags HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme\Components\${SecName}" "Installed"
...
!macro FinishSection SecName
  ...
  IntCmp $AR_SecFlags 1 "leave_${SecName}"
    ...
    WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme\Components\${SecName}" \
  "Installed" 0
    Goto "exit_${SecName}"
...
Function ConditionalAddToRegisty
  ...
  StrCmp "$0" "" ConditionalAddToRegisty_EmptyString
    WriteRegStr SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" \
    "$1" "$0"
...
;Pages
  ...
  !define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\mesytec\mvme"
...
Section "-mvme" SectionMVME
  ...
  WriteRegStr SHCTX "Software\mesytec\mvme" "" $INSTDIR
  ...
  Call ConditionalAddToRegisty
  Push "URLInfoAbout"
  Push "http://www.mesytec.com"
  Call ConditionalAddToRegisty
  Push "Contact"
  Push "info@mesytec.com"
  Call ConditionalAddToRegisty
  ...
  CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\mvme.lnk" "$INSTDIR\mvme.exe"
...
Section "Uninstall"
  ReadRegStr $START_MENU SHCTX \
   "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" "StartMenu"
  ;MessageBox MB_OK "Start menu is in: $START_MENU"
  ReadRegStr $DO_NOT_ADD_TO_PATH SHCTX \
    "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" "DoNotAddToPath"
  ReadRegStr $ADD_TO_PATH_ALL_USERS SHCTX \
    "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" "AddToPathAllUsers"
  ReadRegStr $ADD_TO_PATH_CURRENT_USER SHCTX \
    "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" "AddToPathCurrentUser"
  ;MessageBox MB_OK "Add to path: $DO_NOT_ADD_TO_PATH all users: $ADD_TO_PATH_ALL_USERS"
  ReadRegStr $INSTALL_DESKTOP SHCTX \
    "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme" "InstallToDesktop"
...
!endif
  ...
  DeleteRegKey SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\mvme"

  ;Remove the installation directory if it is empty.
  RMDir "$INSTDIR"

  ; Remove the registry entries.
  DeleteRegKey SHCTX "Software\mesytec\mvme"
  ...
  Delete "$SMPROGRAMS\$MUI_TEMP\mvme.lnk"
  ...
  DeleteRegKey /ifempty SHCTX "Software\mesytec\mvme"
...
inst:
  ...
  StrCmp "$INSTDIR" "$PROGRAMFILES\mvme" 0 +2
  ...
  StrCmp $SV_ALLUSERS "AllUsers" 0 +3
    StrCmp "$IS_DEFAULT_INSTALLDIR" "1" 0 +2
      StrCpy $INSTDIR "$PROGRAMFILES\mvme"
