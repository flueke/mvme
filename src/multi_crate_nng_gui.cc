...
void JobPoller::addJob(const std::shared_ptr<JobContextInterface> &jobContext)
{
    std::lock_guard lock(mutex_);
    jobs_.push_back(jobContext);
}

// FIXME: this is no good. try bulding an executor instead
void JobPoller::loop()
{
    std::vector<std::weak_ptr<JobContextInterface>> jobs;
    std::vector<decltype(jobs)::difference_type> expired;
    ...
    while (true)
    {
        ...
        for (auto it = std::begin(jobs); it != std::end(jobs); ++it)
        {
            if (auto job = it->lock())
            {
                if (job->jobRuntime().isReady() && !job->lastResult().has_value())
                {
                    emit jobReady(job);
                    expired.push_back(std::distance(std::begin(jobs), it));
                }
            }
            else
            {
        ...
        // Reverse to get indexes in descending order.
        std::reverse(std::begin(expired), std::end(expired));

        // Erase in in descending order, keeping remaining indexes valid.
        for (auto index: expired)
        {
            jobs.erase(std::begin(jobs) + index);
        }
