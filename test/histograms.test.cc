...
void dump_histo(const Histo1D &histo)
{
    ...
    auto width  = binning.getBinWidth();
    spdlog::info("Histo1D: {} bins, range [{}, {}], binWidth={}, title={}", binning.getBins(), binning.getMin(),
                 binning.getMax(), width, histo.getTitle().toLocal8Bit().data());

    for (auto bin = 0u; bin < binning.getBins(); ++bin)
    {
        spdlog::info("Bin {} ({}, {}]): {}", bin, binning.getBinLowEdge(bin),
                     binning.getBinLowEdge(bin) + width, histo.getBinContent(bin));
    }
}
...
TEST(Histograms, Histo1DGetCountsInRange)
{
    ...
    ASSERT_DOUBLE_EQ(histo.getBinContent(9), 1.0);
    ...
    ASSERT_DOUBLE_EQ(histo.getCounts(9.0, 10.0), 1.0);
    ...
    ASSERT_NEAR(histo.getCounts(9.2, 9.8), 0.6, epsilon);
    ...
    ASSERT_DOUBLE_EQ(histo.getCounts(6.0, 10.0), 1.0);
    ...
    // range goes outside the histograms range
    ASSERT_DOUBLE_EQ(histo.getCounts(-1.0, 2.0), 1.0);
    ASSERT_DOUBLE_EQ(histo.getCounts(8.0, 12.0), 1.0);
}
...
TEST(Histograms, Histo1DNegativeAxisGetCountsInRange)
{
    ...
    histo.setTitle("Negative Axis Histo");
    ...
    ASSERT_DOUBLE_EQ(histo.getBinContent(0), 1.0);
    ASSERT_DOUBLE_EQ(histo.getBinContent(1), 2.0);
    ASSERT_DOUBLE_EQ(histo.getBinContent(2), 1.0);
    ASSERT_DOUBLE_EQ(histo.getBinContent(3), 2.0);
    ASSERT_DOUBLE_EQ(histo.getBinContent(4), 0.0);
    ASSERT_DOUBLE_EQ(histo.getBinContent(9), 1.0);
    ...
    // range is one bin wide
    ASSERT_DOUBLE_EQ(histo.getCounts(-10.0, -9.0), 1.0);
    ASSERT_DOUBLE_EQ(histo.getCounts(-9.0, -8.0), 2.0);
    ...
    ASSERT_DOUBLE_EQ(histo.getCounts(-1.0, -0.0), 1.0);

    // range is smaller than a bin
    ASSERT_NEAR(histo.getCounts(-9.8, -9.2), 0.6, epsilon);
    ASSERT_NEAR(histo.getCounts(-8.8, -8.2), 1.2, epsilon);
    ASSERT_NEAR(histo.getCounts(-7.8, -7.2), 0.6, epsilon);
    ASSERT_NEAR(histo.getCounts(-6.8, -6.2), 1.2, epsilon);
    ASSERT_NEAR(histo.getCounts(-0.8, -0.2), 0.6, epsilon);

    // range crosses multiple bins
    ASSERT_DOUBLE_EQ(histo.getCounts(-10.0, -8.0), 3.0);
    ASSERT_DOUBLE_EQ(histo.getCounts(-9.0, -7.0), 3.0);
    ...
    ASSERT_DOUBLE_EQ(histo.getCounts(-1.5, -0.0), 1.0);

    // multiple bins with fractional parts
    ASSERT_NEAR(histo.getCounts(-8.8, -8.0), 1.6, epsilon);
    ASSERT_NEAR(histo.getCounts(-8.8, -7.0), 1.6 + 1.0, epsilon);
    ASSERT_NEAR(histo.getCounts(-8.8, -6.0), 1.6 + 1.0 + 2.0, epsilon);
    ASSERT_NEAR(histo.getCounts(-8.8, -6.2), 1.6 + 1.0 + 0.8 * 2.0, epsilon);
}
...
TEST(Histograms, AddHistogramsUnequalBinningsMinBins)
{
    ...
    ASSERT_NEAR(result->getCounts(4.0, 6.0), 1.333, epsilon);
    ASSERT_NEAR(result->getCounts(6.0, 8.0), 0.666, epsilon);
    ASSERT_NEAR(result->getCounts(8.0, 10.0), 0.666, epsilon);
    ASSERT_NEAR(result->getCounts(10.0, 12.0), 0.333, epsilon);
    ASSERT_NEAR(result->getCounts(12.0, 18.0), 0.0, epsilon);
    ASSERT_NEAR(result->getCounts(18.0, 20.0), 1.0, epsilon);
}
