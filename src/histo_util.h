...
class UnitConversionLinearScaleEngine: public QwtLinearScaleEngine
{
  public:
                                    ...
                                    double stepSize = 0.0) const override
    {
        ...
        auto scaleDiv =
            QwtLinearScaleEngine::divideScale(x1, x2, maxMajorSteps, maxMinorSteps, stepSize);
...
class MinBoundLogTransform: public QwtLogTransform
{
  public:
    ...
    virtual QwtTransform *copy() const { return new MinBoundLogTransform(m_minBound); }
...
class AxisBinning
{
  public:
    ...
    inline u32 getBinCount(u32 rrf = NoResolutionReduction) const { return getBins(rrf); }
    ...
    inline double getBinLowEdgeFractional(double binFraction, u32 rrf = NoResolutionReduction) const
    {
    ...
    {
        return (m_nBins == other.m_nBins && m_min == other.m_min && m_max == other.m_max);
    }

    inline bool operator!=(const AxisBinning &other) { return !(*this == other); }
...
struct ResolutionReductionFactors
{
    ...
    {
        return (x == AxisBinning::NoResolutionReduction && y == AxisBinning::NoResolutionReduction);
    }

    inline u32 getXFactor() const { return x == AxisBinning::NoResolutionReduction ? 1u : x; }

    inline u32 getYFactor() const { return y == AxisBinning::NoResolutionReduction ? 1u : y; }
};
...
inline QString make_title_string(const AxisInfo &axisInfo)
{
    QString result;
...
HistoAxisLimitsUI make_axis_limits_ui(const QString &groupBoxTitle, double inputMin,
                                      double inputMax, double limitMin, double limitMax,
                                      bool isLimited);
