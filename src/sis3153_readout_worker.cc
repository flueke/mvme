...
#define SIS_READOUT_BUFFER_DEBUG_FILE   0
#define SIS_READOUT_ENABLE_JUMBO_FRAMES 1
...
namespace
{
    ...
    void stackList_add_block_read(u32 *list_ptr, u32 *list_buffer, u32 vme_addr, u32 vme_nof_bytes, bool mblt, bool fifo)
    {
        ...
        unsigned int vme_access_size = mblt ? 3 : 2;
        unsigned int vme_am_mode     = mblt ? 0x8 : 0x9;
    ...
    QVector<u32> build_stackList(SIS3153 *sis, const vme_script::VMEScript &commands)
    {
        ...
        for (const auto &command: commands)
        {
            ...
            {
                case  CommandType::Read:
                    stackList_add_single_read(&resultOffset, result.data(), command.address, 1, 0x9);
                    break;

                case  CommandType::Write:
                case  CommandType::WriteAbs:
                    stackList_add_single_write(&resultOffset, result.data(), command.address, command.value, 1, 0x9);
                    break;
...
void SIS3153ReadoutWorkerIRQPolling::processReadoutResults(EventConfig *event, s32 eventConfigIndex, const vme_script::ResultList &results)
{
    ...
    *eventHeader |= (eventSize << LF::SectionSizeShift) & LF::SectionSizeMask;
    ...
    debugOutputBuffer(outputBuffer->asU32(0), outputBuffer->used / sizeof(u32));
...
void SIS3153ReadoutWorker::start(quint32 cycles)
{
    ...
    try
    {
        ...
        // FIXME: both methods do not check for errors!
             ...
             ++eventIndex)
        {
            ...
            if (event->triggerCondition == TriggerCondition::Interrupt)
            {
                ...
                // build a command stack and upload it
                // set write start address to  SIS3153ETH_STACK_LIST1_CONFIG
                ...
                qDebug() << __PRETTY_FUNCTION__ << ">>>>> sis stackList for event" << event << ":";
                debugOutputBuffer(stackList.data(), stackList.size());
                qDebug() << __PRETTY_FUNCTION__ << "<<<<< sis stackList:";
                ...
                error = sis->writeRegister(SIS3153ETH_STACK_LIST1_CONFIG + 2 * stackIndex,
                                           stackLoadAddress | ((stackList.size() - 1) << 16)); // XXX: -1 from example code?
                if (error.isError()) throw error;
...
void SIS3153ReadoutWorker::readoutLoop()
{
    ...
    {
        // Qt event processing to handle queued slots invocations (stop, pause, resume)
        // TODO: maybe optimize by calling this only every N loops
        processQtEvents();
        ...
        if (m_state == DAQState::Running && m_desiredState == DAQState::Running)
        {
            ...
            else
            {
                ...
                #if SIS_READOUT_DEBUG
                qDebug("%s: received %u words, packetAck=0x%x, packetIdent=0x%x, packetStatus=0x%x",
                       __PRETTY_FUNCTION__, longwordsRead, (u32)packetAck, (u32)packetIdent, (u32)packetStatus);

                if (longwordsRead >= 4)
                {
                    qDebug("%s: first two words: 0x%08x, 0x%08x", __PRETTY_FUNCTION__,
                           m_readBuffer.asU32(0)[0], m_readBuffer.asU32(0)[1]);

                    qDebug("%s: last two words:  0x%08x, 0x%08x", __PRETTY_FUNCTION__,
