...
void ExpressionOperatorConfigurationWidget::inputSelected(s32 slotIndex)
{
    ...
    if (!wasNameEdited() && required_inputs_connected_and_valid(m_a1_op))
    {
        for (s32 i = 0; i < m_a1_op->getNumberOfSlots(); i++)
        {
            qDebug() << m_a1_op << "slot" << i << ": " << makeSlotSourceString(m_a1_op->getSlot(i));
        }
...
void ExpressionOperatorConfigurationWidget::rebuild()
{
    ...
    {
        qDebug() << e.what() << "symbol =" << e.symbol_name.c_str() << ", is_duplicate =" << e.is_duplicate;
    }
    catch (const a2::ExpressionOperatorSemanticError &e)
    {
        qDebug() << e.what() << "message =" << e.message.c_str();
    }
