...
static void updateHistogramConfigFromFilterConfig(Hist1DConfig *histoConfig, DataFilterConfig *filterConfig)
{
...
void HistogramTreeWidget::editDataFilter(QTreeWidgetItem *node)
{
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        {
            // filter string is unchanged
            // update generated histogram config properties

            for (int childIndex = 0;
                 childIndex < node->childCount();
                 ++childIndex)
            {
                auto histoNode = node->child(childIndex);
