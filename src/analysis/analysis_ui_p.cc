...
struct ExpressionConditionDialog::Private
{
    ...
    void addInputSelectRow(const QString &name = {})
    {
        auto selectButton = new QPushButton(name.isEmpty() ? "<select>" : name);
        selectButton->setCheckable(true);
        selectButton->setMouseTracking(true);
        selectButton->installEventFilter(m_q);
        auto clearButton  = new QPushButton(QIcon(":/dialog-close.png"), QString());
        auto varNameEdit = new QLineEdit;

        int row = m_slotGrid->rowCount();
        m_slotGrid->addWidget(selectButton, row, 0);
    ...
    , m_d(std::make_unique<Private>())
{
    ...
    m_d->m_slotGrid = new QGridLayout(inputsFrame);
    m_d->m_slotGrid->addWidget(new QLabel("Select"), 0, 0);
    m_d->m_slotGrid->addWidget(new QLabel("Clear"), 0, 1);
    m_d->m_slotGrid->addWidget(new QLabel("Variable Name"), 0, 2);
    m_d->m_slotGrid->setColumnStretch(0, 1);
    m_d->m_slotGrid->setColumnStretch(1, 0);
    m_d->m_slotGrid->setColumnStretch(2, 1);

    auto inputsGroupBox = new QGroupBox("Inputs");
    auto inputsGroupBoxLayout = new QGridLayout(inputsGroupBox);
    inputsGroupBoxLayout->setContentsMargins(2, 2, 2, 2);
    inputsGroupBoxLayout->addWidget(inputsFrame, 0, 0, 1, 2);

    auto addInputButton = new QPushButton(QIcon(QSL(":/list_add.png")), QString());
    addInputButton->setToolTip(QSL("Add input"));

    connect(addInputButton, &QPushButton::clicked,
            this, [this] () { m_d->addInputSelectRow(); });

    auto buttonLayout = new QHBoxLayout;
    buttonLayout->setContentsMargins(0, 0, 0, 0);
    buttonLayout->addStretch();
    buttonLayout->addWidget(addInputButton);
    inputsGroupBoxLayout->addLayout(buttonLayout, 1, 0, 1, 2);
    ...
    dialogLayout->addWidget(inputsGroupBox);
